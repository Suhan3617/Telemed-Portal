    ...
    <div className="bg-white shadow hover:shadow-2xl transition-transform transform hover:scale-105 rounded-xl p-5 flex flex-col justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-sm text-gray-500 flex items-center gap-1">
            <CalendarClock size={14}/>{appt.date} • {appt.time}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mt-4">
        {appt.type === "Video Consultation" && (
          <Button className="w-full" onClick={()=>onStartVC?.(appt)}><Video size={16} />&nbsp;Start VC</Button>
        )}
        <button
        ...
        <button onClick={()=>onReschedule?.(appt)} className="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300">
          Reschedule
        </button>
        <button onClick={()=>onCancel?.(appt)} className="px-4 py-2 rounded-lg bg-red-100 text-red-700 hover:bg-red-200">
          Cancel
        </button>
      </div>
    </div>
