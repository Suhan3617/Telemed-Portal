...
const DoctorDashboard = () => {
  ...
  const upcomingFollowUps =
    appointments.filter(
      (a) => a.status === "Scheduled" && a.type.includes("Follow-up")
    ).length || 3; // Placeholder 3
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <motion.div
          // This div now directly wraps the calendar component, providing the card styling and motion.
          className="bg-white rounded-2xl shadow-xl  border border-gray-100 md:col-span-1 lg:col-span-1"
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 1.0, duration: 0.5 }}
        >
          {/* The Calendar component contains its own header and grid structure. */}
          {/* We use it directly here, making sure it doesn't render the header twice. */}
          <Appointmentcalendar />

          {/* Note: If your AppointmentCalendar component already renders the "Appointment Calendar" 
       header (which it does in my previous code), you should remove the h3 tag below 
       to avoid repetition. Assuming you removed the h3 from inside AppointmentCalendar, 
       here's the clean code: */}

          {/* If you prefer to manage the header here: */}
